...
def df_rearrange(
    ...
    df_pivoted = df_copy.pivot_table(values=values, index='Aminoacid', columns=['Position'], dropna=False)
...
def parse_pivot(df_imported: pd.DataFrame, col_variant='variant', col_data='DMS', fill_value=np.nan):
    '''
    ...
    # Get sequence
    sequence = list(df_input.groupby(by=['Position', 'Original'], as_index=False, group_keys=False).sum()['Original'])
