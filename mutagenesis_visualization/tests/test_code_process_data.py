...
BUG THAT NEEDS FIXIN BEFORE THIS'LL RUN
    Under "Make upper case in case input was lower case" replace

    codon_list = [item.upper() for item in codon_list]

    with

    if isinstance(codon_list, str):
        codon_list = codon_list.upper()
    else:
        codon_list = [item.upper() for item in codon_list]

    If you pass in "NNS" it got turned into ["N", "N", "S"]
...
Other bits and bobs:
    ...
    In the else of `if counts_wt:` setting that to np.nan leaves df_counts with
    a mix of NaNs and float 0.0s. I'm not sure if this is a bug or expected
    behavior.
"""

import mutagenesis_visualization.jupyter_notebooks.import_notebook
from mutagenesis_visualization.jupyter_notebooks.code_process_data import (
    count_reads
)
