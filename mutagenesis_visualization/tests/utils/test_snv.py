...
from mutagenesis_visualization.main.utils.snv import (_aminoacids_snv, _dict_codon_to_aa, _codons_pointmutants, _are_pointmutants, _are_pointmutants_list, translate_codons, is_dna)
...
def test_aminoacids_snv() -> None:
    ...
    pairs: List[List[str]] = [['F', 'L'], ['I', 'M'], ['T', 'A'], ['S', 'R'],
         ['F', 'P'], ['I', 'G'], ['T', 'L'], ['S', 'H'], ['A','A']]
    ...
    for pair, expected_answer in zip(pairs, expected_answers):
        calculated_answer = _aminoacids_snv(
            pair[0], pair[1], codon_table, same_aa_SNV=False)
        assert expected_answer == calculated_answer, 'Error in determining SNV'

    # Now change the same_aa_SNV parameter
    assert  _aminoacids_snv('A', 'A', codon_table, same_aa_SNV=True) == True, 'Error in determining SNV when the two amino acids are the same'
...
def test_codons_snv() -> None:
    ...
    # Calculate answer and assert
    for pair, expected_answer in zip(pairs, expected_answers):
        calculated_answer = _codons_pointmutants(
            pair[0], pair[1], same_codon_SNV=False)
        assert expected_answer == calculated_answer, 'Error in determining SNV'

    # Now change the same_aa_SNV parameter
    assert  _codons_pointmutants('CAA', 'CAA', same_codon_SNV=True) == True, 'Error in determining SNV when the two codons are the same'
...
def test_translate_codons() -> None:
    ...
    Testing full capabilities of function.
    """
    list_codons: List[str] = ['AAA', 'AAC', 'AAG', 'AAT', 'ACA', 'ACC', 'ACG', 'ACT', 'AGA', 'AGC', 'AGG', 'AGT', 'ATA', 'ATC', 'ATG', 'ATT', 'CAA', 'CAC', 'CAG', 'CAT', 'CCA', 'CCC', 'CCG', 'CCT', 'CGA', 'CGC', 'CGG', 'CGT', 'CTA', 'CTC', 'CTG',
    ...
    list_aminoacids: List[str] = ['K', 'N', 'K', 'N', 'T', 'T', 'T', 'T', 'R', 'S', 'R', 'S', 'I', 'I', 'M', 'I', 'Q', 'H', 'Q', 'H', 'P', 'P', 'P', 'P', 'R', 'R', 'R', 'R', 'L', 'L',
                       'L', 'L', 'E', 'D', 'E', 'D', 'A', 'A', 'A', 'A', 'G', 'G', 'G', 'G', 'V', 'V', 'V', 'V', '*', 'Y', '*', 'Y', 'S', 'S', 'S', 'S', '*', 'C', 'W', 'C', 'L', 'F', 'L', 'F']
    df_input: DataFrame = pd.DataFrame(index=list_codons)
    assert (translate_codons(df_input) == list_aminoacids), 'error when translating the codons of the dataframe index'
...
def test_is_dna() -> None:
    ...
    assert (is_dna(
        df_1) == False), 'error determining if the index of the dataframe contains DNA'
    assert (is_dna(df_2) ==
